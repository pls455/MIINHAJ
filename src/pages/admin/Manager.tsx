...
export function AdminManager() {
  ...
  useEffect(() => { if (!cfg || !repo) return; setItems([]); setCursor(undefined); setMore(false); setEditing(null); setValues(createEmptyForm(cfg.fields)); setErrors({}); void load(true); }, [domain]);

  useEffect(() => {
    if (domain !== 'resources') return;
    let cancelled = false; setRelationsLoading(true); setError('');
  ...
  async function load(reset = false) { if (!repo) return; setLoading(true); setError(''); try { const page = await repo.list({ pageSize: 20, cursor: reset ? undefined : cursor }); setItems((current) => reset ? page.items : [...current, ...page.items]); setCursor(page.nextCursor); setMore(page.hasMore); } catch (e) { setError(e instanceof Error ? e.message : 'تعذر تحميل البيانات.'); } finally { setLoading(false); } }
  ...
  async function remove(id: string) { if (!repo || !window.confirm('حذف هذا العنصر؟ لا يمكن التراجع عن العملية.')) return; try { await repo.remove(id); await load(true); } catch (e) { setError(e instanceof Error ? e.message : 'تعذر حذف العنصر.'); } }
  ...
  const relationOptions: Record<string, RelationshipOption[]> = { branchId: relations.branches, subjectId: relations.subjects, categoryId: relations.categories };
  return <section>
    ...
    <div className="list" aria-busy={loading}>{filtered.map((item) => { const row = item as BaseDocument & Record<string, unknown>; return <article className="card row" key={item.id}><div><h2>{String(row.title ?? row.name ?? item.id)}</h2><p>{String(row.description ?? row.content ?? '')}</p></div><div className="actions"><button onClick={() => startEdit(item)}>تعديل</button><button className="danger" onClick={() => void remove(item.id)}>حذف</button></div></article>; })}{!loading && !filtered.length && <p className="empty">لا توجد بيانات.</p>}</div>
    {loading && <p>جارٍ التحميل...</p>}{more && !loading && <button className="button" onClick={() => void load()}>تحميل المزيد</button>}
  </section>;
}

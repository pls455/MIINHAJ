...
export function AdminManager() {
  ...
  async function load(reset = false, requestedSearch = search) {
    ...
    try {
      const page = await repo.list({ pageSize: 20, cursor: reset ? undefined : cursor, search: requestedSearch.trim() || undefined });
      setItems((current) => reset ? page.items : [...current, ...page.items]); setCursor(page.nextCursor); setMore(page.hasMore);

import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, limit, orderBy, query, startAfter, updateDoc, where, type DocumentData, type QueryConstraint } from 'firebase/firestore';
...
export function createRepository<T extends BaseDocument>(name: string) {
  return {
    async get(id: string) {
      ...
      return snapshot.exists() ? mapDoc<T>(snapshot) : null;
    },
    async list(options: ListOptions = {}): Promise<Page<T>> {
      ...
      if (options.cursor) {
        ...
        constraints.push(startAfter(cursor.order, cursor.id));
      }
    ...
    async create(data: Record<string, unknown>): Promise<string> { const result = await addDoc(collection(db, name), data); return result.id; },
    async update(id: string, data: Record<string, unknown>): Promise<void> { await updateDoc(doc(db, name, id), data); },

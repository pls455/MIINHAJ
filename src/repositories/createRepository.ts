...
function addBranchConstraint(constraints: QueryConstraint[], options: Pick<ListOptions, 'branchId'|'branchIds'>): void {
  if (options.branchIds?.length) constraints.push(where('branchIds', 'array-contains-any', options.branchIds.slice(0, 10)));
  else if (options.branchId) constraints.push(where('branchId', '==', options.branchId));
}
...
export function createRepository<T extends BaseDocument>(name: string) {
  return {
    ...
    async count(options: Pick<ListOptions, 'active'|'branchId'|'branchIds'|'subjectId'|'categoryId'> = {}) {
      const constraints: QueryConstraint[] = [];
      if (options.active !== undefined) constraints.push(where('active', '==', options.active));
      addBranchConstraint(constraints, options);
    ...
    async list(options: ListOptions = {}): Promise<Page<T>> {
      ...
      return { items, nextCursor: last ? encodeCursor({ value: lastValue, id: last.id }) : undefined, hasMore: items.length === size };
    },
    async create(data: Record<string, unknown>): Promise<string> { const payload = { ...data, searchNormalized: searchableText(data), createdAt: serverTimestamp(), updatedAt: serverTimestamp() }; return (await addDoc(collection(db, name), payload)).id; },

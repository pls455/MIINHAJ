...
export async function requireAdmin(requiredRole = ROLES.REVIEWER) { const { user, admin } = await requireAuthenticatedAdmin(); const normalizedAdmin = { ...admin, role: normalizeRole(admin.role) }; if (!hasRole(normalizedAdmin.role, requiredRole)) { const error = new Error('INSUFFICIENT_PERMISSIONS'); error.code = 'INSUFFICIENT_PERMISSIONS'; throw error; } return { user, admin: normalizedAdmin }; }
export { ROLES };

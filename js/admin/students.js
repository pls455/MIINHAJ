...
function render() {
  ...
  const total=totalCount??students.length; const today=todayCount??students.filter(s=>{const d=dateValue(s.createdAt);const t=new Date();t.setHours(0,0,0,0);return d&&d>=t;}).length; const withIdentity=identityCount??students.filter(s=>s.identityNumber).length;
  content.innerHTML=`<div class="stat-grid"><article class="stat"><span>إجمالي الطلاب</span><b>${Number(total).toLocaleString('ar-EG')}</b></article><article class="stat"><span>المسجلون اليوم</span><b>${Number(today).toLocaleString('ar-EG')}</b></article><article class="stat"><span>نتيجة البحث</span><b>${filtered.length.toLocaleString('ar-EG')}</b></article><article class="stat"><span>الطلاب ذوو الهوية</span><b>${Number(withIdentity).toLocaleString('ar-EG')}</b></article></div><div class="toolbar"><input id="studentSearch" type="search" placeholder="ابحث بالاسم أو البريد أو رقم الهوية أو UID" value="${esc(term)}"><button id="exportStudents" class="button" type="button">تصدير CSV</button></div><div class="section-head"><h2>سجل الطلاب</h2><span class="message">تم تحميل ${students.length.toLocaleString('ar-EG')} من ${Number(total).toLocaleString('ar-EG')} طالب</span></div><div class="admin-table">${rows||'<div class="loading">لا توجد نتائج.</div>'}</div>${hasMore&&!term?'<div class="toolbar"><button id="loadMoreStudents" class="button button--secondary" type="button">تحميل المزيد</button></div>':''}`;

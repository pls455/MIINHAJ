...
function render() {
  ...
  content.innerHTML = `
    ...
    <div class="section-head"><h2>سجل الطلاب</h2><span class="message">${total ? `تم تحميل ${total.toLocaleString('ar-EG')} طالب` : 'لا توجد حسابات طلاب بعد'}</span></div>
...
function exportCsv() {
  ...
  const lines=[headers,...students.map(s=>headers.map(h=>{const v=h.endsWith('At')?dateText(s[h]):(s[h]??'');return String(v).replaceAll('"','""');}))].map(row=>row.map(v=>`"${v}"`).join(','));

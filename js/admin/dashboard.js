...
function card(label, value) {
  const article = document.createElement('article'); article.className='admin-stat';
...
function sectionLinks(admin) {
  const nav=document.createElement('nav'); nav.className='admin-nav-grid'; nav.setAttribute('aria-label','أقسام الإدارة');
  const links=[];
  if (permissions.system(admin.role)) links.push(['students.html','الطلاب المسجلون']);
...
async function load(){
  try{
    ...
    const counts=await Promise.all([
      ...
      permissions.system(admin.role) ? getDocs(collection(db,'users')) : Promise.resolve({size:0})
    ]);
    const [branches,subjects,categories,resources,foundations,users]=counts;
    const stats=document.createElement('div');stats.className='admin-stats';
    [['الطلاب المسجلون',users.size],['الفروع',branches],['المواد',subjects],['التصنيفات',categories],['المصادر',resources],['التأسيس',foundations]].forEach(([label,value])=>stats.append(card(label,value)));
    content.replaceChildren(stats,sectionLinks(admin));
  }catch(error){
    console.error('[admin-dashboard]',error);
    const box=document.createElement('div');box.className='error-box';box.setAttribute('role','alert');box.textContent='تعذر التحقق من صلاحيات الإدارة أو تحميل الإحصائيات.';content.replaceChildren(box);
  }
}
load();

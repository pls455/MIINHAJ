...
function setStatus(text, type='loading') {
  if (!content) return;
  content.innerHTML = `<div class="${type === 'error' ? 'error-box' : 'loading'}" role="status">${esc(text)}</div>`;
}

function normalizeRole(role) {
  const value = String(role || '').trim().toLowerCase();
  if (['superadmin','super_admin','admin','super-admin','مدير النظام','المدير العام'].includes(value)) return 'super_admin';
  if (['contentadmin','content_admin','content-admin','مدير المحتوى'].includes(value)) return 'content_admin';
  if (['reviewer','مراجع','مراجع المصادر'].includes(value)) return 'reviewer';
  return value;
}
...
function exportCsv() {
  ...
  const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`minhaj-students-${new Date().toISOString().slice(0,10)}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url),1000);
}
...
async function safeCounts() {
  ...
  if (results[0].status === 'fulfilled') totalCount = results[0].value.data().count;
  if (results[1].status === 'fulfilled') todayCount = results[1].value.data().count;
  if (results[2].status === 'fulfilled') identityCount = results[2].value.data().count;
}
...
async function fetchPage(after = null) {
  ...
  try {
    ...
    const snap = await getDocs(query(users, limit(PAGE_SIZE)));
    lastDoc = null;
    hasMore = false;
    return snap.docs.map(d => ({id:d.id, ...d.data()}));
  }
...
async function loadMore() {
  ...
  try {
    ...
    console.error('[admin-students-v2] load more', error);
    if (button) { button.disabled = false; button.textContent = 'تعذر التحميل، أعد المحاولة'; }
  } finally { loadingMore = false; }
}

async function getAdminAfterAuth(user) {
  ...
  const admin = { uid: snapshot.id, ...snapshot.data() };
...
function waitForAuth(timeoutMs = 12000) {
  ...
  return new Promise((resolve, reject) => {
    let done = false;
    let unsubscribe = () => {};
    const finish = (fn, value) => { if (done) return; done = true; clearTimeout(timer); unsubscribe(); fn(value); };
    const timer = setTimeout(() => { const e = new Error('AUTH_TIMEOUT'); e.code = 'AUTH_TIMEOUT'; finish(reject, e); }, timeoutMs);
...
async function boot() {
  try {
    ...
    const admin = await getAdminAfterAuth(user);
    identity.textContent = `${user.email || admin.email || 'حساب الإدارة'} • Super Admin`;
    const firstPage = await fetchPage();
    students = firstPage;
    render();
    safeCounts().then(render).catch(error => console.warn('[admin-students-v2] counts', error));
  } catch (error) {
    console.error('[admin-students-v2] boot', error);

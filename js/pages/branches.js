...
async function load(reset = true) {
  ...
  try {
    ...
    if (page.hasMore) { const next = document.createElement('button'); next.className='button'; next.textContent='التالي'; next.onclick=async()=>{cursor=page.nextCursor;await load(false)}; pagination.append(next); }
  } catch (e) { console.error(e); list.innerHTML='<div class="error-box">تعذر تحميل الفروع.</div>'; }
}
await load();

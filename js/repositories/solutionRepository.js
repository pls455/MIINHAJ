...
function normalizeLimit(value) {
  return Math.min(Math.max(Number(value) || 24, 1), MAX_LIMIT);
}
...
export async function getSolutions({ pageSize = 24, cursor = null, subjectId = null, activeOnly = false } = {}) {
  ...
  constraints.push(orderBy('order', 'asc'), limit(normalizeLimit(pageSize)));
...
export async function getSolution(id) {
  const snap = await getDocs(query(collectionRef(), where('__name__', '==', id), limit(1)));
...
export async function createSolution(data) {
  ...
  const refDoc = await addDoc(collectionRef(), {
    ...normalized,
  ...
  await writeAdminLog({
    ...
    collectionName: NAME,
    targetId: refDoc.id,
    details: { title: normalized.title },
  });
  return refDoc;
}
